...
describe("speech model registry", () => {
  it("keeps Parakeet as the default and offers Qwen3-ASR and Whisper", () => {
    ...
    expect(getSpeechModel("whisper-turbo").modelId).toBe("turbo");
  });

  it("validates values loaded from local storage or IPC", () => {
    expect(isSpeechModelId("qwen3-asr-0.6b")).toBe(true);
    expect(isSpeechModelId("whisper-turbo")).toBe(true);
